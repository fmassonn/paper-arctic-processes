...
for j_models in range(n_models):
  ...
  ax[0].scatter(x, y1, 20 * n_members, color = colors_complexity[comp], alpha = 0.5)
  ax[0].text(   x, y1, str(j_models + 1).zfill(2), color = [c / 2.0 for c in colors_complexity[comp]], fontsize = 4, ha = 'center', va = 'center')
  ax[1].scatter(x, y2, 20 * n_members, color = colors_complexity[comp], alpha = 0.5)
  ax[1].text(   x, y2, str(j_models + 1).zfill(2), color = [c / 2.0 for c in colors_complexity[comp]], fontsize = 4, ha = 'center', va = 'center')
  del x, y1, y2
  ...
  for j_members in range(n_members):
    ...
    ax[0].scatter(x, y1, 0.5, color = colors_complexity[comp])
    ax[1].scatter(x, y2, 0.5, color = colors_complexity[comp])
...
ax[1].set_xlabel("Annual-mean volume [10$^3$ km$^3$]")
...
plt.tight_layout()
plt.savefig("./figED4.png", dpi = 500)
